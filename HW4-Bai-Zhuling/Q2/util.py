...
def partition_classes(X, y, split_attribute, split_val):
    ...
    if str(x).isdigit():
        for item in X:
            if item[split_attribute] > split_val:
                num_right = X.index(item)
                X_right.append(item)
                y_right.append(y[num_right])
            else:
                num_left = X.index(item)
                X_left.append(item)
                y_left.append(y[num_left])
    else:
        for item in X:
            if item[split_attribute] == split_val:
                num_right = X.index(item)
...
def information_gain(previous_y, current_y):
    ...
    for item in current_y:
        h += entropy(item)*len(item)/len(previous_y)
    info_gain = H - h
    return info_gain
